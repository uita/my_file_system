...
#define id_equal(node) ((node == NULL ? 0 : ((struct tq_node*)node)->id == id))
        //getn_from_two_list(node, id_equal, tq);
        remove_container(id_equal, node, tq->ls1, struct tq_node, var);
        if (!node)
                remove_container(id_equal, node, tq->ls2, struct tq_node, var);
        //
        if (node) {
                add_rcount(node);
        } else {
